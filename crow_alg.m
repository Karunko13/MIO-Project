...
function g_best = crow_alg(myObj)


    pd=20; % Problem dimension (number of decision variables)
    N=size(myObj.getcurrentParams(),2); % Flock (population) size
    AP=0.1; % Awareness probability
    fl=2; % Flight length (fl)
    
    [x,l,u]=init(N,pd, myObj.getcurrentParams()); % Function for initialization
    xn=x;
    ft=fitness(xn,N,pd,myObj); % Function for fitness evaluation
    
    mem=x; % Memory initialization
    fit_mem=ft; % Fitness of memory positions
    
    tmax=5; % Maximum number of iterations (itermax)
    for t=1:tmax
        t
        ...
        xn=xnew;
        ft=fitness(xn,N,pd,myObj); % Function for fitness evaluation of new solutions
    
        for i=1:pd % Update position and memory
